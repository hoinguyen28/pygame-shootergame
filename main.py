...
class Soldier(pygame.sprite.Sprite):
    def __init__(self, char_type, x, y, scale, speed, ammo, grenades):
        ...
        for animation in animation_type:
            # reset temporaty list of images
            temp_list = []
    ...
    def update(self):
        ...
        self.check_alive()
        # update cooldown
        if self.shoot__cooldown > 0:
            self.shoot__cooldown -= 1
    ...
    def move(self, moving_left, moving_right):
        ...
        # jump
        if self.jump == True and self.fly == False:
            self.vel_y = -11
            self.jump = False
            self.fly = True
        # apply gravity
    ...
    def shoot(self):
        if self.shoot__cooldown == 0 and self.ammo > 0:
            self.shoot__cooldown = 20
...
class Grenade(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.timer <= 0:
            ...
            # do damege to anyone that is nearby
            ...
            if abs(self.rect.centerx - player.rect.centerx) < TILE_SIZE * 2 and abs(self.rect.centery - player.rect.centery) < TILE_SIZE * 2:
                player.health -= 50
            if abs(self.rect.centerx - player.rect.centerx) < TILE_SIZE and abs(self.rect.centery - player.rect.centery) < TILE_SIZE:
                player.health -= 100
            for enemy in enemy_group:
...
item_box = ItemBox('Ammo', 200, 260)
item_box_group.add(item_box)
item_box = ItemBox('Grenade', 300, 260)
item_box_group.add(item_box)


player = Soldier('player', 200, 200, 2, 3, 20, 5)
health_bar = HealthBar(10, 10, player.health, player.health)
enemy = Soldier('enemy', 400, 250, 2, 5, 20, 0)
    ...
    if player.alive:
        ...
        # throw grenades
            ...
            grenade = Grenade(player.rect.centerx + (0.3 * player.rect.size[0] * player.direction),
                              player.rect.top * 1.1, player.direction)
